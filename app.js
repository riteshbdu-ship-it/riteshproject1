...
function updateDashboardFromFilteredData(filtered) {
  ...
  if (!filtered || (filtered.length === 0 && !hasActiveFilter)) {
    const all = Explorer.allRecords || [];
    if (all.length) return updateDashboardFromFilteredData(all);
    return;
  }
  ...
  principlesPieChartInst = new Chart(ctx, {
    ...
    plugins: [{
      ...
      afterDraw(chart) {
        ...
        ctx.fillText(RW_DATA.totalRecords.toLocaleString(), cx, cy - 4);
...
document.addEventListener('DOMContentLoaded', async () => {
  const records = await (window.driveDataReady || Promise.resolve(RW_DATA.mockRecords || []));
  if (records.length && typeof finishLoad === 'function') finishLoad(records, Object.keys(records[0]), null, null, null);
  const kpis = {
    kpiTotal: RW_DATA.totalRecords,
    kpiPaperMill: records.filter(r => /paper mill/i.test(r.Reason || '')).length,
    kpiAI: records.filter(r => /ai|computer-generated|chatgpt|generative/i.test(r.Reason || '')).length,
    kpiCountries: new Set(records.flatMap(r => String(r.Country || '').split(';').map(v => v.trim()).filter(Boolean))).size,
    kpiJournals: new Set(records.map(r => r.Journal).filter(Boolean)).size,
    kpiInvestigations: records.filter(r => /investigation|inquiry/i.test(r.Reason || '')).length
  };
  Object.entries(kpis).forEach(([id, value]) => {
    const element = document.querySelector(`#${id} .kpi-val`);
    if (element) element.dataset.count = value;
  });
  animateCounters();
  initOverview();
  initExplorer();
